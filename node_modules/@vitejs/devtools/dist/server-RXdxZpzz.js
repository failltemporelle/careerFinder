import { n as diagnostics, r as logger, t as MARK_INFO } from "./constants-CwdrFfgU.js";
import { t as createDevToolsMiddleware } from "./server-BHMAegNY.js";
...
const $ = w("│", "|"), x = w("└", "—");
const z = w("●", ">"), U = w("○", " "), st = w("─", "-"), ct = w("╮", "+"), Gt = w("├", "+"), $t = w("╯", "+"), dt = w("╰", "+"), ht = w("●", "•"), pt = w("◆", "*"), mt = w("▲", "!"), gt = w("■", "x"), P = (t) => {
	switch (t) {
		case "initial":
...
`), n = u.reduce((o, l) => Math.max(fastStringWidth(l), o), 0);
	...
	const r = s?.output ?? process$1.stdout, u = s?.withGuide ?? h.withGuide, n = s?.format ?? we, a = [
		"",
		...be(t, A(r) - 6, n).split(`
`).map(n),
		""
...
`), d = u ? `${styleText("gray", $)}
` : "", g = u ? Gt : dt;
	r.write(`${d}${styleText("green", H)}  ${styleText("reset", i)} ${styleText("gray", st.repeat(Math.max(o - c - 1, 1)) + ct)}
${l}
